...
struct ReservationPage: View {
    var body: some View {
        AsyncContentView {
            try await SportsReservationStore.shared.getVenues()
        } refreshAction: {
            try await SportsReservationStore.shared.getVenues(forceRefresh: true)
        } content: { venues in
            SportsVenueList(venues: venues)
        }
...
/// A network-free reservation screen for app-level visual verification.
public struct ReservationDemoPage: View {
    public init() {}

    public var body: some View {
        NavigationStack {
            SportsVenueContent(
                venue: ReservationDemoData.venue,
                detail: ReservationDemoData.detail,
                testCalendar: ReservationDemoData.calendar,
                reservationInfoVisibilityOverrides: ReservationDemoData.reservationInfoVisibility,
                initialDate: ReservationDemoData.date
            )
            .navigationTitle(ReservationDemoData.venue.name)
            .navigationBarTitleDisplayMode(.inline)
        }
    }
}
...
private struct SportsVenueContent: View {
    ...
    let testCalendar: BookingCalendar?
    let reservationInfoVisibilityOverrides: [Int: Bool]
    ...
    init(
        venue: BookingVenue,
        detail: BookingVenueDetail,
        testCalendar: BookingCalendar? = nil,
        reservationInfoVisibilityOverrides: [Int: Bool] = [:],
        initialDate: Date = .now
    ) {
        self.venue = venue
        self.detail = detail
        self.testCalendar = testCalendar
        self.reservationInfoVisibilityOverrides = reservationInfoVisibilityOverrides
    ...
    var body: some View {
        List {
            ...
            if introduction != nil || bookingDescription != nil {
                Section {
                    if let introduction {
                        Text(introduction.plainText)
                            .font(.subheadline)
                    ...
                    if let bookingDescription {
                        Button {
                            presentedSheet = .rules(bookingDescription)
                        } label: {
                    ...
                    if introduction != nil {
                        Label(bookingLocalized("Venue Introduction"), systemImage: "building.2")
                    }
                }
            }
        ...
        .sheet(item: $presentedSheet) { sheet in
            ...
            case let .rules(description):
                BookingTextSheet(
                    title: bookingLocalized("Reservation Rules"),
    ...
    private var introduction: BookingRichText? {
        guard let introduction = detail.introduction,
              introduction.isEnabled,
              !introduction.plainText.isEmpty else {
            return nil
        }
        return introduction
    }

    private var bookingDescription: BookingRichText? {
        guard let bookingDescription = detail.bookingDescription,
              bookingDescription.isEnabled,
              !bookingDescription.plainText.isEmpty else {
            return nil
        }
    ...
    @ViewBuilder
    private func periodRows(for calendar: BookingCalendar) -> some View {
        if calendar.periods.isEmpty {
            ...
            ForEach(calendar.periods) { period in
                BookingPeriodRow(
                    ...
                    showsReservationInfo: reservationInfoVisibilityOverrides[period.id]
                )
            }
        }
    }
...
private struct BookingPeriodRow: View {
    ...
    private let showsReservationInfoOverride: Bool?
    ...
    init(
        ...
        showsReservationInfoOverride = showsReservationInfo
    }
    ...
    private var showsReservationInfo: Bool {
        showsReservationInfoOverride ?? (availableCount < calendar.courts.count)
    }
        ...
        HStack(alignment: .center, spacing: 16) {
            VStack(alignment: .leading, spacing: 4) {
                HStack(alignment: .firstTextBaseline, spacing: 10) {
                    ...
                    if showsReservationInfo {
                        Button(action: onShowReservations) {
                            Image(systemName: "info.circle")
                                .font(.body)
                        }
                        .buttonStyle(.plain)
                        .foregroundStyle(.secondary)
                        .accessibilityLabel(bookingLocalized("Reservation Info"))
                    }
                }
            ...
            Button(availableCount > 0 ? bookingLocalized("Reserve") : bookingLocalized("Full"), action: onReserve)
...
private struct BookingTextSheet: View {
    @Environment(\.dismiss) private var dismiss
    let title: String
    let text: String
...
#if !os(watchOS)
private struct CachedVenueImage: View {
    let venue: BookingVenue
    ...
    var body: some View {
        ...
        .clipped()
        .task(id: venue.images) {
            imageData = nil
            imageData = try? await SportsReservationStore.shared.getVenueImage(venue)
        }
    }
}
#endif
...
private enum ReservationDemoData {
    static let date = bookingCalendar.date(from: DateComponents(year: 2026, month: 9, day: 13))!

    static let venue: BookingVenue = decode(
        """
        {
          "id": 1001,
          "name": "北区体育馆-羽毛球",
    ...
    static let detail: BookingVenueDetail = decode(
        """
        {
          "id": 1001,
          "name": "北区体育馆-羽毛球",
          ...
          "config": {
            "data_colle": [],
            "introduction": {
              "text": "北区体育馆位于北区学生公寓 73 号楼旁，设有标准与非标准羽毛球场。预约签到地点为一楼服务台。",
              "label": "",
              "enanle": true
            },
            "description": {
              "text": "请在预约时段开始前到场签到；如无法到场，请及时取消预约。",
              "label": "",
              "enanle": true
            },
            "anti_bot": 0
    ...
    /// Four states: full/info, full/no-info, available/info, and available/no-info.
    static let reservationInfoVisibility: [Int: Bool] = [1: true, 2: false, 3: true]

    static let calendar: BookingCalendar = decode(
        """
        {
          "day": ["2026-09-13"],
          "time": [
            { "id": 1, "str_time": "08:00–09:00", "start_time": "08:00", "end_time": "09:00" },
            { "id": 2, "str_time": "09:00–10:00", "start_time": "09:00", "end_time": "10:00" },
            { "id": 3, "str_time": "10:00–11:00", "start_time": "10:00", "end_time": "11:00" },
            { "id": 4, "str_time": "11:00–12:00", "start_time": "11:00", "end_time": "12:00" }
          ],
          "resource": [
            { "id": 1, "name": "1号场地" },
            { "id": 2, "name": "2号场地" }
          ...
          "data": {
            "2026-09-13": {
              "1": {
                "1": { "status": 2, "total": 2, "num": 0 },
                "2": { "status": 2, "total": 2, "num": 0 },
                "3": { "status": 0, "total": 2, "num": 1 },
                "4": { "status": 0, "total": 2, "num": 1 }
              },
              "2": {
                "1": { "status": 2, "total": 2, "num": 0 },
                "2": { "status": 2, "total": 2, "num": 0 },
                "3": { "status": 2, "total": 2, "num": 0 },
                "4": { "status": 0, "total": 2, "num": 1 }
              }
            }
          }
