...
struct MentionView: View {
    ...
    private var localMention: some View {
        ...
        .previewContextMenu(preview: ScrollView { THFloorView(floor: floor).padding() }) {
    ...
    private var remoteMention: some View {
        ...
        #if os(iOS)
        // TODO: use geometry reader to determine size
        .previewContextMenu(destination: THDetailPage(holeId: floor.holeId, floorId: floor.id),
                            preview: ScrollView { THFloorView(floor: floor).padding() })
        #endif
    }
