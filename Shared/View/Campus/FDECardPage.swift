...
struct FDECardPage: View {
    @State var balance = ""
    @State var page = 1
    @State var records: [FDTransaction] = []
    ...
    func initialFetch() async throws {
        balance = try await FDECardAPI.getBalance()
    ...
    func loadMoreRecords() async {
        do {
            ...
            let newRecords = try await FDECardAPI.getTransactions(page: page)
...
fileprivate struct FDTradeRecordView: View {
    let record: FDTransaction
