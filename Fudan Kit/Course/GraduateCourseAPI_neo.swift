...
public enum GraduateCourseAPI_neo {
    ...
    ///       "KSJCDM": 2,
    ///       "JSXM": "教师姓名",
    ///       "BJDM": "2025202602POLI70016.01"
    ///     }
    ///   ]
    /// }
    /// ```
    ///
    /// - Returns: Course list decoded from `results`.
    public static func getCourses() async throws -> [Course] {
        ...
        let builders = responses.compactMap(parseCourse)
        return mergeCourse(builders)
    }

    private static func parseCourse(_ response: CourseResponse) -> CourseBuilder? {
