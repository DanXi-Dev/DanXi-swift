...
public actor GraduateCourseStore {
    ...
    public func getSemesterInfo() async throws -> ([Semester], Semester?) {
        let (_, semester) = try await getCachedCourse()
        return ([semester], semester)
    }

    public func getRefreshedCourses() async throws -> ([Course], Semester) {
        let (courses, fetchedSemester) = try await GraduateCourseAPI.getCourses()
...
/*
 以下是原研究生课表小程序 Store 实现。
 由于对应数据源更新不及时，现已弃用；代码保留在这里，便于后续追溯或参考。
